...
#Alter, Geschlecht, Überlebenschancen
print(passengers %>% group_by(Age, Sex)) %>% summarise(Survived = sum(Survived))
age_sex_surv <- (passengers %>% group_by(Age, Sex)) %>% summarise(Survived = sum(Survived))
ggplot(data = age_sex_surv, mapping = aes(x = Sex, y = Survived, fill = as.factor(Age))) +
